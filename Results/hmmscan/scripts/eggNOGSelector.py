...
def filterDF(fileName:str,threshold=50):
    ...
    if df.empty:
        ...
        df=df.loc[:,['target_name', 'query_name']]
        return df,{orgName:df.shape[0]}

def worker(_input_:str,_output_:str,threshold=50):
    df,result_dict=filterDF(_input_,threshold=threshold)
    if df.empty:
        return result_dict
    else:
...
if __name__=='__main__':
    configFile='/data/irsingh/Zinc_Work/WoL_reset_18Nov2025/Results/hmmscan/scripts/eggNOGSelector.config'
